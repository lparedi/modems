...
if "debug" in argsdict:
    ...
    debug = 0
if "noread" in argsdict:
    actions["ReadMissing"] = 0
...
if actions["ToDo"] == 1:
    if actions["S_LP0"] == 1:
        ...
        time.sleep(5)
        # time.sleep(90)
    if actions["ReadOnSummer"] == 0 and (int(date.today().strftime("%m")) > 5 and int(date.today().strftime("%m")) < 10  ) :
        MyModem.sendprecomputed("23232341542b534348443d342d303b")
        MyModem.sendprecomputed("23232341542b534348443d342d303b")
        actions["ReadMissing"] == 0
    else:
    ...
    #prevent single reading before a full read is performed
    currentday = date.today().strftime("%d")
    if int(currentday) < 16 and actions["ForceRead"] == 0:
        actions["ReadMissing"] = 0
    #today = "202203"
        ...
        while missing == 1 and count < 5:
            ...
            if actions["UseSQL"] == 1:
                if actions["FakeReadingsDate"] == 0:
                    ...
                    TelegramToSql(meterslist, MyModem.Telegrams, str(actions["FakeReadingsDate"]), actions["MbusRecordCurrentValue"], actions["MbusMixedRecords"],
                              mixedrecordsdict, actions["PyMbus"], MyModem.Mbresults)
                if actions["FakeReadingsDate"] == 0:
                    readedfromsql = getmetersreaded(RealEstates, today,actions["OverwriteCondID"])
                else:
                    readedfromsql = getmetersreaded(RealEstates, str(actions["FakeReadingsDate"]),actions["OverwriteCondID"])
            ...
            for hca in meterslist:
                ...
                if actions["UseSQL"] == 1:
                    if str(line) not in readedfromsql  and   str(line) not in actions["IgnoredList"]  :
                        if actions["StrictMbus"] == 1 or len(MbusDict) != 0:
                            ...
                            # sendprecomputed(precomputed, modemnumber)
                            time.sleep(5)
                            precomputed = MyModem.computetelegram("SELECT_FD", str(line))
                            MyModem.sendprecomputed(precomputed)
                            time.sleep(5)
