...
class Uit:
    ...
    def __init__(self,condid,condata,errors):
        ...
        self.errors = errors
def ElaborateErrors(condid , type):
    if len(sys.argv) == 1:
    ...
    if type == "modem":
        ...
        if not condata:
            print("Nessun Dato Condominio")
            exit(0)
        #pprint(condata[0]['Città'])
        #continue
        #exit(0)
        mir  = getmetersinerror(condid,str(today))
        
        readedfromsql = commonfunctions.getmetersreaded([str(condid)], str(today),0)
